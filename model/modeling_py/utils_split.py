...
Preprocessing Steps:
- Assumes input features (X) are provided as a pandas DataFrame.
- Assumes target labels (y) are provided as a pandas Series.
...
def get_stratified_kfold_splits(X: pd.DataFrame, y: pd.Series, n_splits: int = 5, random_state: int = 42):
    ...
    # Generate and return a list of (train_index, test_index) tuples for each fold
