...
df_outlier = df.copy()
numeric_cols = df_outlier.select_dtypes(include=['float64', 'int64']).columns.tolist()
if target_col in numeric_cols:
    numeric_cols.remove(target_col)

z_scores = np.abs(zscore(df_outlier[numeric_cols]))
threshold = 3
outlier_mask = (z_scores > threshold).any(axis=1)
print(f"❗ 이상치 행 수: {outlier_mask.sum()}개")

df_no_outlier = df_outlier[~outlier_mask]
...
if existing_onehot_cols:
    df_encoded = pd.get_dummies(df, columns=existing_onehot_cols, drop_first=True)
